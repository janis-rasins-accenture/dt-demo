...
const WithThemeProvider = (Story: StoryFn, context: StoryContext) => {
  ...
  const isInvertedVariant = variant === 'inverted' || variant === 'inverse';
  const isInverseCondition = isInDocs
    ? context.id.toLowerCase().includes('inverted') || context.id.toLowerCase().includes('inverse')
    : isInverse === true;

  const background =
    isInvertedVariant || isInverseCondition ? '#262626' : currentTheme.colors['page-bg-color-full'];

  return (
    <ThemeProvider brand={brand} mode={mode}>
      <StyledDiv
        bg={isSheetComponent ? currentTheme.colors['page-bg-color-subtle'] : background}
        inDocs={isInDocs}
      >
        <Story {...context} />
      </StyledDiv>
